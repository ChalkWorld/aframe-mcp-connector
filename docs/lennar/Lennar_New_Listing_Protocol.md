...
**Airtable is the source of truth** for internal listing tracking (model, photos, POC, Aframe ID, session notes, MLS input stage) and for all community reference data (schools, HOA, fees, heating/pool/amenities). The **Google Sheet main tab remains authoritative for Status, Current Price, and Closing Date only** — these three fields are periodically refreshed into Airtable from the sheet, not the other way around. See Systems & Reference below for the full breakdown.
...
**Airtable tool reference** — `tool_search` must be called before any of these load; exact phrase noted where the obvious phrase doesn't surface it.
...
| `typecast: true` | param on create/update records calls | Auto-adds a new option to a `singleSelect`/`multipleSelects` field if the value being written isn't in its choice list yet. Default `false`. Use when writing a Status (or similar) value that might be new |
| Raw Google Sheets read (bypasses Zapier's unreliable search actions) | `Zapier:execute_zapier_read_action`, action `_zap_raw_request`, tool `google_sheets_make_api_get_request` | Zapier's `get_data_range` / `get_many_rows` actions returned silently empty results on a populated worksheet (2026-08-14) even though the connection and metadata reads worked fine. Use this raw passthrough to hit `https://sheets.googleapis.com/v4/spreadsheets/{id}/values/'{Sheet Name}'!A1:Z###` directly instead of trusting the abstracted read actions |
...
**Session implication:** the POC field on the Airtable Lennar Listings table (Step 7) should be the actual rep who submitted the listing, not a generic contact — confirm from the intake email/thread rather than defaulting to a name.
...
- **Duplicate check** — search the Airtable Lennar Listings table (by MLS# or address) and Aframe before creating anything. Reps have submitted duplicates before.
- **List date is always today** — never the email date or form submission date.
- **Google Sheet main tab is read-only for sessions** — the primary tab is visible externally to Lennar sales contacts. Sessions read it to check Status/Current Price/Closing Date and refresh those fields into Airtable; never write to the sheet. Andrew handles all writes to the main tab manually. The Airtable Lennar Listings table is the session's write target for everything else — no narration required before writing there, unlike the old Session Data tab convention.
...
- Set `"community"` to the exact community display name (e.g. `"Harpers Mill TH"`) — the session uses this to look up per-community values in the **Airtable Community Reference DB table** (schools, HOA fees, management firm, capital contributions) and in `Lennar_Payload_Schema.md` §5.2 Features B community table (heating, heat fuel, pool, community amenities — not yet migrated to Airtable as of this writing).
...
Add a corresponding row to the **Airtable Lennar Listings table**:
...
- [ ] Save MLS# back to the Airtable Lennar Listings table once assigned; surface to Andrew for manual entry on the Google Sheet main tab
...
3. Update Current Price and Price Change Date in the Airtable Lennar Listings table
...
2. Update Status and Closing Date in the Airtable Lennar Listings table
