...
| 3.2 | 2026-08-07 | Andrew Rich / Claude | Batch/Bulk Operations promoted from Tier 2 to Tier 1 on v0.7.0 build. Final tool set: `bulk_update_custom_fields`, `bulk_add_transaction_participants`, `bulk_search_contacts` — `bulk_create_contacts` cut (no standalone use case; the dominant workflow always adds a contact as a participant, and `bulk_add_transaction_participants` already creates-and-links inline), `bulk_search_contacts` added (co-op agent/lender/closer lookup case). Tier 2 Batch/Bulk Operations section removed accordingly. |
...
Each tool loops an existing single-entity function server-side and returns one aggregated result — Aframe still receives one HTTP call per entity (no batch endpoint exists on their side), but the loop moves below the LLM boundary instead of costing a tool-call round trip per entity. Partial-failure tolerance follows the same pattern proven in `apply_task_templates`: a bad entry is skipped and reported, the whole batch only fails if every entry fails. `bulk_search_contacts` is the exception — read-only, so every entry simply returns its match count with no failure state. Design finalized in a dedicated build session; see `SESSION-HANDOFF-2026-08-07-batch-tools.md` and `BATCH-TOOLS-DESIGN-001`. `bulk_create_contacts` from the original three-tool plan was cut — the dominant workflow never needs a contact created without also being added as a participant, and `bulk_add_transaction_participants` already creates-and-links inline. `bulk_search_contacts` was added instead, covering the co-op agent / lender / closer lookup case — contacts that might already exist from a past deal, the same thing Aframe's own UI typeahead checks for.

| Tool | Wraps | Released |
|---|---|---|
| `bulk_update_custom_fields` | Loops `update_custom_field` | v0.7.0 |
| `bulk_add_transaction_participants` | Loops `add_transaction_participant` | v0.7.0 |
| `bulk_search_contacts` | Loops `search_contacts` | v0.7.0 |
