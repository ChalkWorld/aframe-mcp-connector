...
**Rules-table scope philosophy landed explicitly (v0.8).** Midway through Bath Info's first authoring pass this session, Andrew's vibe check surfaced drift: the Rules table had been quietly moving toward defensive derivation — property-type splits to correct for probable NHC input mistakes, Transforms gating on `LevelsWithBathrooms.Contains(...)` to hedge against Cognito's stale-value behavior, prose verbs like "populated" that deferred script-side decisions to the Python author. Correct scope landed as the reference for the remaining tabs: the Rules table maps Form 17 → Matrix as direct passthrough. It does not second-guess NHC input, error-correct for rep mistakes, or apply defensive derivations. Bad NHC input is a form-side or NHC-training concern, handled upstream; missing Matrix-required fields caught by Cognito's own required-field validation. DERIVED is for legitimate value differences (path scoping counts, Matrix-quirk value logic like "TS" when FullBath > 0), not for defensive rewrites. The extension-optimization running list (LockBox Type, Input_611) dropped — Rules table absence IS the optimization signal; no separate list needed. Full framing in `SESSION-HANDOFF-2026-09-17-PAYLOAD-RULES-PHILOSOPHY-AND-TABS-5-7.md`.

**Payload_Rules_Conventions.md v0.1 created and dropped in `/docs/foundation/`.** `PAYLOAD-RULES-CONVENTIONS-001`. Houses five base-level conventions the script author reads once and treats as base-level guarantees: (1) numeric Matrix TEXT count fields — null/missing/empty → `"0"`; (2) the Cognito Field link is the primary source for COGNITO rules, gating logic on other fields is DERIVED; (3) Transform / Logic is the definitive spec when present, verbatim passthrough when blank; (4) STATIC rows with empty Static Value are deliberate empty writes, distinct from no-row; (5) fields the script never touches have no row. Individual rules do not restate these. Change Log at the bottom for future additions. Payload Rules table description update via API was blocked repeatedly ("No approval received"); conventions file is the reference until Andrew pastes them into the table description manually.

**Bath Info 15 rules torn down and rewritten as 12 clean rules.** Original v0.7-parked authoring produced 15 rules with property-type splits (SF/TH on Basement Full/Half/Description) and gate-check Transforms (`IF LevelsWithBathrooms.Contains("Level 1") AND Level1.FullBath > 0 → ...`). Post-philosophy rewrite: 8 count fields as COGNITO passthrough with no Transform (convention #1 handles null → `"0"`); 4 description fields as DERIVED with Transform `IF Level<n>.FullBath > 0 → write "TS". ELSE write empty string.` — the derivation is Matrix's own value-rejection rule, not defensive derivation. Level 4 excluded (no Form 17 source). Basement no longer split by property type. `BedsBathsLevels.Section.LevelsWithBathrooms` Cognito Fields row deleted (gate isn't referenced under new philosophy). Remaining 8 bath-count Cognito Fields rows had Notes columns tightened to drop stale-value alarm language. Cognito Fields' `Conditional Visibility` field description updated to strip prescriptive language — it's descriptive metadata now, not a directive to the script.

**Virtual Tour Info authored: 1 rule.** `Input_610` COGNITO from `PhotosVirtualTour.VirtualTourLink` — verbatim URL passthrough. Payload-level tab-skip behavior (extension skips the entire tab if URL is blank) captured in the rule's Notes column. `Input_611` (additional URL) has no Form 17 source and no row per the "absence of a row" pattern — the schema doc lists two payload keys but Form 17 only provides one. First tab of the pass exercising "fewer rules than the schema doc's key count."

**General Info authored: 8 rules.** 5 both-path STATIC (Waterfront `"N"`, Model Available `"0"`, Disclosures `["NOTREQ"]`, Lead Disclosure `["NOTREQ"]`, Assd Improvement `"0"`). 3 new-path-only (Acres COGNITO from `PropertyBasics.Acres` with decimal→string Transform + blank passthrough; Tax Year DERIVED from script-runtime clock; Legal Description STATIC `"TBD"`). First path-scoped rules of the authoring pass. Path multiselect column exercised cleanly for both-path (`["new", "taxid"]`) and new-only (`["new"]`) shapes. First DERIVED-from-system-context rule pattern established (Tax Year → `str(datetime.now().year)`) — validates that DERIVED works for compute-from-runtime rules without any convention change.

**Payload Schema §4.4 Tax Year correction queued.** Currently documented as STATIC `"0"` on new path; corrected per Andrew 2026-09-17 to DERIVED current calendar year at script execution. Joins the existing correction queue (New/Resale, Year Built Description, SqFt Source defaults, `LBGD`→`LGD`). Fold-in still deferred to a dedicated correction pass.

**Property Type field carries orphan options.** Multi-select has `SF`, `TH` (original) plus `Single Family`, `Townhouse` (added by a typecast call on the abandoned Bath Basement property-type-split rules). No live rule uses the latter two. Deletion of multi-select options is not supported via the Airtable connector — Andrew removes them in-app at his convenience.

**Payload Rules coverage: 7 of 12 tabs complete — 37 rows written.** Owner Info (7 STATIC), Agent/Office Info (2 STATIC), Internet Display Info (4 STATIC), Showing Instructions (2 STATIC + 1 COGNITO), Virtual Tour Info (1 COGNITO), Bath Info (8 COGNITO + 4 DERIVED), General Info (5 STATIC + 1 COGNITO + 1 DERIVED + 1 STATIC path-scoped — 3 new-only). Room Info explicitly out of scope. 4 tabs remaining: Fee Info, Remarks, Listing Info, Features.

**Fee Info authored: 10 rules (v0.9).** 2 STATIC both-path (HOA/Condo `"1"`, Membership Reqd `"1"`), 7 COMMUNITY_DB both-path (Fee Amount, Fee Period, Management Firm, Fee Description, Fee Includes, Add'l Fee Amount, Add'l Fee Desc), 1 DERIVED both-path (Add'l HOA Y/N — inspects Additional Fee column presence). First rules with Source Type = COMMUNITY_DB and first population of the Community DB Column field on Payload Rules (field already existed on the table; anticipated this session's use during v0.7 base build). First DERIVED rule reading from a Community DB column rather than a Cognito field. First Transform-based display-text-to-checkbox-code crosswalk (Fee Description `"Community Association"` → `["01"]`, `"Condo Association"` → `["02"]`). First Transform-based combined-string extraction (HOA Fee `"$800.00 / Yearly"` split across two rules — Fee Amount → `"800.00"`, Fee Period → `"YR"` — reading the same DB column with different Transforms). Management Phone (Input_706) and Allow Onsite excluded (no row) — Matrix defaults blank, extension currently writes unnecessarily (same category as v0.7's LockBox Type observation).

**Remarks authored: 3 rules (v0.9).** 2 COGNITO passthrough (Public Remarks → Input_107 from `Remarks.PublicRemarks`, Agent-Only Comments → Input_108 from `Remarks.AgentOnlyComments`), 1 STATIC (Copyright Agreement → Input_662 `"1"`). Cognito Field records added ahead of the rules via the Cognito connector's `get_form_schema` on Form 17 — cleaner than reconstructing paths from doc content. Phone stripping (previous session-based intake compliance behavior — strip phone/call lines from Public Remarks per CVRMLS rule) explicitly excluded from script scope: Form 17's Remarks section helptext already instructs NHCs not to include the Sales Hotline in Public Remarks. Legacy email-sourced intake still handles this at session time; Form 17 intake trusts rep compliance. Documented in the Public Remarks rule Notes.

**Capital contribution scope open question resolved as blanket rule (v0.9).** Payload Schema §7.1's "confirmed for 2 of 5 communities" question about writing capital contribution to `fee.addl_fee_desc` (Input_117): reframed correctly midway through the session — it was never about the cap contribution *amounts* (those had been in the DB for months), it was about whether the write to Input_117 should be blanket vs. scoped to only-verified communities. Resolved per Andrew's call to blanket-write for all communities. Reasoning: Lennar surfaces capital contribution consistently across all community HOA materials, so writing it for all communities is the natural default; a future community without one becomes the exception, handled by the DB's Capital Contribution column returning blank and passing through cleanly. §7.1 doc text update deferred to the batched Schema corrections pass.

**Path-in-DB architectural decision (v0.9 — implementation deferred to next session).** Path designation currently lives in Payload Schema §3 and Protocol Entry Path Rules — not the Community DB. Andrew's "why isn't path just in the DB? Seems like the most logical place to put it" surfaced the mismatch: path is per-community data (same shape as MLS Area, Zip, Property Type) that had been living in doc space rather than data space. Decision to move Path to a `singleSelect` field on the Community Reference DB (options: `new`, `taxid`); Schema §3 and Protocol Entry Path Rules become pointers at the DB as source of truth (migration narrative retained for provenance). Everstone SF's path status change (new → taxid, tax records now populated — parallel to Creekside's August 2026 migration) is absorbed into this same rearchitect step rather than done as a one-off Schema §3 update that would immediately get superseded.

**Copyright Agreement precedent: MLS-wide constant at Builder layer (v0.9).** Copyright Agreement (Input_662) is a CVRMLS-wide constant (all CVRMLS listings write `"1"`) — not Lennar-specific. Included in Lennar Payload Rules as STATIC so the Phase 1 script writes a complete Lennar payload without extension-side hardcoding gaps. Rule Notes flag: when a second builder is added to this stack, decide whether to promote to an MLS-layer shared enum vs. copy per-builder. Nuances (but does not override) v0.7's excluded-field policy — Matrix flags Input_662 if blank, so it MUST be written; that's script-side work regardless of layer categorization.

**Fee Description community-scoping contradicts Confirmed Lennar-Wide Static Values (v0.9 — queued for Protocol correction).** `Lennar_New_Listing_Protocol.md`'s "Confirmed Lennar-Wide Static Values" table lists Fee Desc as always `Input_111_01` (Community Association) — a Lennar constant. Contradicted by Creekside Run using Condo Association (`Input_111_02`) — the value varies by community and is sourced via COMMUNITY_DB in the Rules table. Second time authoring the Rules table has caught a doc-vs-reality gap (first was v0.7's LockBox Type STATIC-blank → excluded reclassification). Queued for correction in the next Protocol-updates pass.

**Payload Rules coverage: 9 of 12 tabs complete — 50 rows written (v0.9).** Owner Info (7 STATIC), Agent/Office Info (2 STATIC), Internet Display Info (4 STATIC), Showing Instructions (2 STATIC + 1 COGNITO), Virtual Tour Info (1 COGNITO), Bath Info (8 COGNITO + 4 DERIVED), General Info (5 STATIC + 1 COGNITO + 1 DERIVED + 1 STATIC path-scoped — 3 new-only), Fee Info (2 STATIC + 7 COMMUNITY_DB + 1 DERIVED — 10 rules), Remarks (2 COGNITO + 1 STATIC — 3 rules). Room Info explicitly out of scope. 2 tabs remaining: Listing Info, Features. Cognito Fields at 13 total (2 new this session — `Remarks.PublicRemarks`, `Remarks.AgentOnlyComments`).

### Decisions Parked for Next Session

**Payload Rules authoring — 2 tabs remaining.** This session added Fee Info (10 rules) and Remarks (3 rules). Remaining tabs in ascending complexity: Listing Info (biggest, community-name resolution derivation, County/City/schools cascades, path-scoped fields — will exercise the just-added DB Path field for the first time in a Rules-authoring context), Features (biggest and most complex, includes Payload Schema §5.4.1 Cognito crosswalks and all the Lennar Feature statics). Room Info explicitly out of scope.

**Path-in-DB architectural implementation** (new, high priority — see Decisions Landed above). Add a `Path` singleSelect field to the Community Reference DB Airtable table (base `app78fMUwDNBHUZ6r`, table `tbleMbM1WgY8Si2t7`), options `new` and `taxid`. Populate for Active records: HM TH → `taxid`, HM SF → `taxid`, Creekside Run → `taxid`, Everstone → **`taxid`** (status change this session; Everstone parcels' tax records populated), Watermark → `new`. Wynwood retired, leave blank. Update `Lennar_Payload_Schema.md` §3 and `Lennar_New_Listing_Protocol.md` Entry Path Rules to point at the DB as source of truth (retain migration narrative for provenance, including the Creekside 2026-08-11 and Everstone 2026-09-18 migrations).

**Cascade Options table build.** Unchanged from v0.6 — design agreed, not yet created in Airtable. Still parked while Payload Rules authoring is the active line.
...
**Six Payload Schema value corrections queued.** New/Resale (`RESAL`), Year Built Description (`ACTUAL`), SqFt Source `01`/`02` swap, Showing Instructions `LBGD`→`LGD`, Tax Year "0" → DERIVED current calendar year (queued v0.8), and §7.1 capital contribution scope → blanket write for all communities (queued v0.9 — resolves the "confirmed for 2 of 5" open question). Fold-in still deferred to a dedicated correction pass. Payload Schema §4.7-4.11 imprecisions surfaced in v0.7 remain deliberately not queued (schema stays lean per that session's policy call).

**One Protocol correction queued (v0.9).** `Lennar_New_Listing_Protocol.md`'s "Confirmed Lennar-Wide Static Values" table lists Fee Desc as always `Input_111_01` (Community Association); contradicted by Creekside Run using `Input_111_02` (Condo Association) — value varies by community and is sourced via COMMUNITY_DB in the Rules table. Fold in during the next Protocol-updates pass.

**HOA Fee / Additional Fee DB column split** (new, future DB improvement — low priority). Both columns in the Community Reference DB currently store combined display strings (`"$800.00 / Yearly"`). This session's Fee Info rules use Transform-based extraction (see Decisions Landed); splitting each column into Amount and Period would make the Transforms pure passthrough. Not blocking script development.

**Copyright Agreement multi-builder promotion decision** (new, low priority). When a second builder is added to this stack, decide whether to promote Copyright Agreement (currently STATIC `"1"` in Lennar Payload Rules) to an MLS-layer shared enum vs. copy per-builder. The rule's own Notes flag this same question.
...
**Payload Rules table description update via API blocked.** Unchanged from v0.8. Conventions live in `Payload_Rules_Conventions.md` (in `/docs/foundation/`) instead; Andrew can paste them into the base description manually if he wants them mirrored.
...
**Payload_Rules_Conventions.md ID convention decision.** Unchanged from v0.8. Small decision on whether to rename to `FOUND-CONVENTIONS-001` (or similar) and update any downstream references.

---

## Next Session Opening Move

1. Read this tracker first, then `SESSION-HANDOFF-2026-09-18-FEE-INFO-AND-REMARKS.md` for full session detail on Fee Info + Remarks authoring, the path-in-DB architectural decision, and the MLS-wide-constant-at-Builder-layer precedent. Also read `Payload_Rules_Conventions.md` (in `/docs/foundation/`) before touching Rules authoring — it's the base-level reference the remaining tabs are authored against.
2. **Path-in-DB architectural implementation** (highest priority — architectural decision landed v0.9, implementation deferred). Add a `Path` singleSelect field to the Community Reference DB (base `app78fMUwDNBHUZ6r`, table `tbleMbM1WgY8Si2t7`), options `new` and `taxid`. Populate for Active records: HM TH → `taxid`, HM SF → `taxid`, Creekside Run → `taxid`, Everstone → **`taxid`** (status change from v0.9 — Everstone's parcels now have tax records populated, parallel to Creekside's August 2026 migration), Watermark → `new`. Wynwood retired, leave blank. Update `Lennar_Payload_Schema.md` §3 and `Lennar_New_Listing_Protocol.md` Entry Path Rules to point at the DB as source of truth (retain migration narrative for provenance).
3. **Continue Payload Rules authoring — 2 tabs remaining.** Suggested order in ascending complexity: Listing Info (biggest, community-name resolution derivation, County/City/schools cascades, path-scoped fields — will exercise the just-added DB Path field for the first time in a Rules-authoring context) → Features (biggest and most complex, includes Payload Schema §5.4.1 Cognito crosswalks and all the Lennar Feature statics). Room Info explicitly out of scope.
4. **Build the Cascade Options table** (still parked from v0.6 — design agreed, needs creating).
5. **Fold the six queued Payload Schema value corrections** into `Lennar_Payload_Schema.md` — New/Resale (`RESAL`), Year Built Description (`ACTUAL`), SqFt Source `01`/`02` swap, Showing Instructions `LBGD`→`LGD` (in the Showing Instructions reference doc), Tax Year "0" → DERIVED current year (queued v0.8), and §7.1 capital contribution scope → blanket write for all communities (queued v0.9). Payload Schema §4.7-4.11 imprecisions surfaced in v0.7 remain deliberately not queued.
6. **Fold the queued Protocol correction** into `Lennar_New_Listing_Protocol.md` — Fee Desc entry in the Confirmed Lennar-Wide Static Values table (queued v0.9).
7. **Resolve whether Features has real Matrix UI Section panels** — unchanged open item; a quick live-DOM check settles it either way.
8. Confirm Post Office / Subdivision closure with Andrew if not already settled.
9. Property Type multi-select orphan option cleanup (Andrew, in-app).
10. Decide whether to rename `Payload_Rules_Conventions.md`'s ID from `PAYLOAD-RULES-CONVENTIONS-001` to a `FOUND-*` prefix.
11. Room Info stays deferred — Lennar skips this tab entirely.
...
| 0.8 | 2026-09-17 | Session close after Payload Rules authoring pass 2: philosophy landing + 3 tabs (Virtual Tour Info, Bath Info, General Info). Started as a straight continuation of v0.7's parked queue but midway through Bath Info's first-pass authoring surfaced drift: Rules had been moving toward defensive derivation (property-type splits to correct for probable NHC input mistakes, gate-check Transforms hedging against Cognito stale-value behavior, imprecise verbs like "populated" deferring script-side decisions to the Python author). Correct scope landed explicitly: Rules table maps Form 17 → Matrix as direct passthrough; script does not second-guess NHC input, error-correct for rep mistakes, or apply defensive derivations. Bad NHC input handled upstream (Cognito form-side validation, NHC training); missing Matrix-required fields caught by Cognito's own required-field validation. Extension-optimization running list dropped — Rules table absence IS the optimization signal. Bath Info's 15 rules torn down and rewritten as 12 clean rules. Established base-level `Payload_Rules_Conventions.md` v0.1 (dropped in `/docs/foundation/`) housing five conventions: null → "0" for numeric-count Matrix fields, Cognito Field link is primary source, Transform is definitive spec, STATIC blank = deliberate write, no-row = script doesn't touch. Path scoping first exercised on General Info (5 both-path + 3 new-only). First DERIVED-from-system-context rule pattern established (Tax Year → current calendar year at script runtime — joins the queued Payload Schema correction list). Payload Rules table description update via API blocked ("No approval received"); conventions file is the reference until Andrew pastes manually. Coverage now 7 of 12 tabs, 37 rules. 4 tabs remaining: Fee Info, Remarks, Listing Info, Features. Full detail in `SESSION-HANDOFF-2026-09-17-PAYLOAD-RULES-PHILOSOPHY-AND-TABS-5-7.md`. |
| 0.9 | 2026-09-18 | Session close after Payload Rules authoring pass 3: Fee Info (10 rules) + Remarks (3 rules) = 13 rules added. Session opened with fresh Lennar-direct fee data (HM SF cap $300, HM TH $350, Creekside $450, Everstone $144, Wynwood pending — later confirmed stale on Lennar's own sheet, Wynwood retirement holds), Everstone SF path status change (new → taxid — tax records populated), and the parked "cap contribution scope confirmed for 2 of 5 communities" open question dropped into scope. Two architectural decisions of longer-term consequence than the tabs themselves: (1) path-in-DB — path designation moves from Payload Schema §3 and Protocol Entry Path Rules to a `singleSelect` field on the Community Reference DB (same shape as any other per-community lookup); Schema §3 and Protocol become pointers at the DB as source of truth with migration narrative retained; Everstone path status change absorbed into this same rearchitect step; implementation deferred to next session. (2) MLS-wide constant at Builder layer — Copyright Agreement (Input_662) is CVRMLS-wide but included as STATIC in Lennar Payload Rules so the script writes a complete Lennar payload without extension-side hardcoding gaps; Notes flag future multi-builder promotion decision (shared MLS-layer enum vs. copy per-builder). Nuances (not overrides) v0.7's excluded-field policy — Matrix flags Input_662 if blank, so the write is script-side work regardless of layer categorization. Cap contribution scope resolved as blanket-write for all communities (Andrew's call — Lennar surfaces cap contribution consistently across all community HOA materials). §7.1 doc text update batched into the queued Schema corrections pass. Rules-table philosophy from v0.8 held under real pressure: phone-stripping in Public Remarks explicitly excluded from script scope (Form 17 section helptext already instructs NHCs — Rules table trusts rep compliance, legacy email-sourced intake still handles it at session time); Add'l HOA Y/N modeled as DERIVED reading the Additional Fee column presence (legitimate derivation, not defensive). Fee Description community-scoping (Creekside = Condo Association) surfaced a Protocol contradiction with the Confirmed Lennar-Wide Static Values table — second time authoring the Rules table has caught a doc-vs-reality gap; queued for Protocol correction. First rules with Source Type = COMMUNITY_DB (7 rules); first DERIVED reading from a Community DB column rather than Cognito (1 rule); first Transform-based text-to-code crosswalk (Fee Description); first Transform-based combined-string extraction (HOA Fee `"$800.00 / Yearly"` → Amount + Period as two rules over the same DB column). HOA Fee / Additional Fee DB column split flagged as low-priority future improvement. Cognito Fields added via `get_form_schema` on Form 17 rather than reconstructing from doc content. Coverage now 9 of 12 tabs, 50 rules. 2 tabs remaining: Listing Info, Features. Full detail in `SESSION-HANDOFF-2026-09-18-FEE-INFO-AND-REMARKS.md`. |
