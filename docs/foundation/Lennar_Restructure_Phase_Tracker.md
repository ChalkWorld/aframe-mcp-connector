...
**Payload Rules authoring — 4 tabs remaining.** This session added Virtual Tour Info, Bath Info, and General Info (3 tabs, +21 rules net after Bath teardown/rewrite). Remaining tabs in ascending complexity: Fee Info (first COMMUNITY_DB rules, first community-scoped rows; carries the Payload Schema's "capital contribution scope only confirmed for 2 of 5 communities" open question), Remarks (Cognito with character-limit constraints), Listing Info (biggest, community-name resolution derivation), Features (biggest and most complex, includes Payload Schema §5.4.1 Cognito crosswalks). Room Info explicitly out of scope.
...
**Five Payload Schema value corrections queued.** New/Resale (`RESAL`), Year Built Description (`ACTUAL`), SqFt Source `01`/`02` swap, Showing Instructions `LBGD`→`LGD`, and Tax Year "0" → DERIVED current calendar year (added this session). Fold-in still deferred to a dedicated correction pass. Payload Schema §4.7-4.11 imprecisions surfaced in v0.7 remain deliberately not queued (schema stays lean per that session's policy call).
...
**Payload Rules table description update via API blocked.** `update_table` returned "No approval received" repeatedly this session, even on retry. Conventions live in `Payload_Rules_Conventions.md` (dropped into `/docs/foundation/`) instead; Andrew can paste them into the base description manually if he wants them mirrored.

**Property Type multi-select orphan option cleanup.** Field has `SF` / `TH` (in use) plus `Single Family` / `Townhouse` (unused, added by a typecast pass on the abandoned Bath Basement rules). Airtable connector cannot delete multi-select options; Andrew removes in-app at his convenience.

**Payload_Rules_Conventions.md ID convention decision.** File uses `PAYLOAD-RULES-CONVENTIONS-001`; foundation-doc convention is `FOUND-*` prefix. Small decision on whether to rename to `FOUND-CONVENTIONS-001` (or similar) and update any downstream references.

---

## Next Session Opening Move

1. Read this tracker first, then `SESSION-HANDOFF-2026-09-17-PAYLOAD-RULES-PHILOSOPHY-AND-TABS-5-7.md` for the full philosophy discussion and the Virtual Tour / Bath Info / General Info authoring detail. Also read `Payload_Rules_Conventions.md` (in `/docs/foundation/`) before touching Rules authoring — it's the base-level reference the remaining tabs are authored against.
2. **Continue Payload Rules authoring — 4 tabs remaining.** Suggested order in ascending complexity: Fee Info (first COMMUNITY_DB rules, first community-scoped rows; carries the Payload Schema's capital-contribution scope question) → Remarks (Cognito with character-limit constraints) → Listing Info (biggest, community-name resolution derivation) → Features (biggest and most complex, includes Payload Schema §5.4.1 Cognito crosswalks). Room Info explicitly out of scope.
3. **Build the Cascade Options table** (still parked from v0.6 — design agreed, needs creating).
4. **Fold the five queued Payload Schema value corrections** into `Lennar_Payload_Schema.md` — New/Resale (`RESAL`), Year Built Description (`ACTUAL`), SqFt Source `01`/`02` swap, Showing Instructions `LBGD`→`LGD` (in the Showing Instructions reference doc), and Tax Year "0" → DERIVED current year (added this session). Payload Schema §4.7-4.11 imprecisions surfaced in v0.7 remain deliberately not queued.
...
7. Property Type multi-select orphan option cleanup (Andrew, in-app).
8. Decide whether to rename `Payload_Rules_Conventions.md`'s ID from `PAYLOAD-RULES-CONVENTIONS-001` to a `FOUND-*` prefix.
9. Room Info stays deferred — Lennar skips this tab entirely.
