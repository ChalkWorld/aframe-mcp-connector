# Cursor Handoff — Update CURSOR_INSTRUCTIONS.md to v1.1

**Type:** Meta-handoff (not an endpoint extraction)
**Target file:** `handoffs/CURSOR_INSTRUCTIONS.md`
...
This is a one-off update to `handoffs/CURSOR_INSTRUCTIONS.md` itself. Do **not** process this through the normal endpoint-handoff workflow (it has no `<!-- TARGET -->` for an endpoint).

### Step 1 — Replace the file

Replace the entire contents of `handoffs/CURSOR_INSTRUCTIONS.md` with the content in the **"New File Content"** section below (everything between the `=== BEGIN ===` and `=== END ===` markers, exclusive of those markers themselves).

### Step 2 — Verify

After replacement, confirm:
- Front matter `version` reads `1.1`
- Front matter `version_date` reads today's date (2026-06-14)
- A new row exists in the Version History table noting the v1.1 changes
- The "Two handoff paths" section is present
- The "User Procedure — Path B (raw-paste handoff)" section is present
...
```
Cursor instructions: v1.1 — add raw-paste handoff support

- Add Path B: user pastes raw Swagger Schema content directly into a
  handoff file with section markers; Cursor formats into the template
  before applying
- Preserve Path A (pre-formatted handoffs from Claude.ai sessions)
  for backward compatibility
- Add explicit User Procedure with copy/paste steps
- Add Cursor logic for path detection and section parsing
- Update failure-modes table for Path B edge cases
```
...
### Step 5 — Archive this handoff

Move this file to `handoffs/applied/` with today's date prefixed (e.g., `2026-06-14_update-cursor-instructions-v1.1.md`).
